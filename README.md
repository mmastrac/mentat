...
```
docker pull mmastrac/mentat-artifacts:0.7.0
```

To build every image locally:

```
VERSION=0.7.0 ./build.sh
```
...
```dockerfile
COPY --from=mmastrac/mentat-artifacts:0.7.0 /out/mentatd /usr/local/bin/mentatd
COPY --from=mmastrac/mentat-artifacts:0.7.0 /out/mentatd-0.7.0-py3-none-any.whl /tmp/
