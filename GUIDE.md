...
```bash
docker pull mmastrac/mentat-artifacts:0.2.2
```

Or build them yourself, as the compose deployment does:

```bash
VERSION=0.2.2 ./build.sh
```
...
```dockerfile
COPY --from=mmastrac/mentat-artifacts:0.2.2 /out/mentatd /usr/local/bin/mentatd
COPY --from=mmastrac/mentat-artifacts:0.2.2 /out/mentatd-0.2.2-py3-none-any.whl /tmp/
...
entrypoint. `ray --version` reports `ray, version 2.57.0 (mentatd 0.2.2)`, and
