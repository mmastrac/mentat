...
```bash
docker pull mmastrac/mentat-artifacts:0.2.1
```
...
```dockerfile
COPY --from=mmastrac/mentat-artifacts:0.2.1 /out/mentatd /usr/local/bin/mentatd
COPY --from=mmastrac/mentat-artifacts:0.2.1 /out/mentatd-0.2.1-py3-none-any.whl /tmp/
...
entrypoint. `ray --version` reports `ray, version 2.57.0 (mentatd 0.2.1)`, and
